...
def build_result_payload(
    raw_result: dict[str, Any], file_name: str, content_type: str,
    file_size_bytes: int, processing_time_ms: int,
) -> dict[str, Any]:
    ...
    for index, image in enumerate(page_images):
        ...
        pages.append({
            ...
            "detections": [_detection_entry(d, index, width, height) for d in detections if d.get("page_number") == page_number],
    ...
    return {
        ...
        "processing": {"status": raw_result.get("status", "FAILED"), "duration_ms": processing_time_ms},
        "documents": [document],
        "summary": {"total_documents": 1, "total_pages": len(pages)},
...
def _detection_entry(detection: dict[str, Any], page_index: int, width: int | None, height: int | None) -> dict[str, Any]:
    return {
        ...
        "bounding_box": _bbox(detection.get("bounding_box"), detection.get("normalized_bounding_box"), width or detection.get("page_width"), height or detection.get("page_height")),
...
def _bbox(pixel: Any, normalized: Any, width: int | None, height: int | None) -> dict[str, Any] | None:
    ...
    if normalized is None and width and height:
        normalized = [round(values[0] / width, 6), round(values[1] / height, 6), round(values[2] / width, 6), round(values[3] / height, 6)]
